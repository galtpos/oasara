...
export default function MusicRoute() {
  ...
  if (!catalog) return (
    <>
      <SiteHeader />
      <div style={{ minHeight: '60vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: brand.backgroundColor }}>
        <div style={{ color: brand.textColor }}>Loading music...</div>
      </div>
    </>
  );
  return (
    <>
      <SiteHeader />
      <MusicProvider brandConfig={brand} catalog={catalog}>
        <MusicPage />
        <MusicBar />
      </MusicProvider>
    </>
